---
name: dk-system
description: Domain Knowledge 建立系統。當用戶說「我接了一個新專案」、「幫我做 Kickoff」、「幫我建立邏輯地圖」、「週五到了幫我更新邏輯地圖」、「幫我跑 DK」、「我剛 Kickoff 完」、「幫我維護邏輯地圖」、「解釋這段」、「評估替代方案」、「整合舊記錄」、「這個 bug 同時影響」、「幫我準備向主管報告」、「突發狀況」、「進度落後」、「依賴卡住」、「這個需求算不算加太多」，或任何與新專案開始、邏輯地圖建立、每週維護、技術內容解釋、往主管報告、時程/資源/範圍變動相關的需求時，一律使用這個 skill。
---
...
### 模式九：約束變動（時程 / 資源 / 範圍）
專案的三大約束在執行中途變動時使用。三種觸發共用同一個處理邏輯：讀檔 → 評估影響 → 更新 report.md → 產出溝通草稿。

觸發 A：進度落後（時程）——「進度落後：[什麼卡住了]」
1. 讀 logic_map/專案名稱_report.md 的時程和依賴
2. 評估這個延誤會連帶影響哪些 Phase / 假設
3. 更新 report.md 的時程欄位
4. 產出給主管的 heads-up 草稿（用模式七的 Bottom line 格式）

觸發 B：依賴卡住（資源）——「依賴卡住：[哪個依賴]」
1. 讀 report.md 找出這個依賴影響哪些功能 / 假設
2. 判斷是否進入關鍵路徑（會不會拖累整個專案）
3. 建議是否需要升級處理（escalate 給主管）
4. 記進 log.md

觸發 C：需求追加（範圍）——「這個需求算不算加太多：[新需求]」
1. 讀 logic_map/專案名稱_kickoff.md 的「不做的是」邊界
2. 判斷新需求是否超出原本範圍
3. 如果超出，量化影響（多花多少時間 / 影響哪個 Phase）
4. 產出可以跟 PM 談的說法（延後、或換掉別的既有項目）

共通：處理完都要把結論記進 log.md（加【專案名稱】標籤）。
...
| dk-system/references/situations.md | 模式八、模式九和遇到困難情境時 |
