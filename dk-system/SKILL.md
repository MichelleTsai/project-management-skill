...
## GitHub 設定
- Repository：project-management-skill
- 邏輯地圖路徑：logic_map/
...
- 採用指標：每個專案在 report.md 中需定義至少一個 Primary 指標（例如：每週節省工時、採用率）。建議同時定義 Secondary 指標以衡量品質或穩定性。
- 採用監測：每週或每次自動產出時記錄採用狀態（有無被工程師下載/使用），以便定期評估採用率。
- 回退條件：若採用率低於預設門檻（例如 50% 或你指定的閾值），執行回退流程：暫停自動化輸出、通知相關 stakeholder、安排示範與快速修正會議。
- 維運責任：在 report.md 中標註維運 owner（團隊或角色），並在發現資料 schema 變動時啟動變更通告流程。

注意：上述內容為通用操作準則，請依專案情況在每個 report.md 中具體化數值與負責人。文件中不會包含任何測試情境的私人或人名細節。
...
| references/situations.md | 遇到困難情境時，找到對應情境並引導解決 |
...
| references/step4_sop.md | 模式二和模式三，建立或更新邏輯地圖時 |
| references/situations.md | 遇到困難情境時，找到對應情境並引導解決 |
>>>>>>> 3eee4f3 (docs: normalize skill situational coverage and convert README checklist to table)
